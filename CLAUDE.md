...
- **Attention tracking** (`lib/use-needs-attention.ts`): Tracks pending permission requests AND completed instances (running → idle). All badges are red (pulsing dot in chat list, count on nav). Auto-dismiss when user opens `/chats` (timestamp stored in localStorage `hub_seen_attention`). Missed completions (app opened after Claude finished, within 30 min) also detected. Attention items sorted to top in chat list.
