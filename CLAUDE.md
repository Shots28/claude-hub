...
- **Attention tracking** (`lib/use-needs-attention.ts`): Only tracks **pending permission requests** — completions are not tracked (they're visible from status badges and aren't actionable). Red dot badge on nav + red pulsing dot in chat list for instances needing approval. All badges auto-dismiss when the user opens `/chats` (timestamp stored in localStorage `hub_seen_attention`). Permissions created before the last-seen timestamp are not shown. Attention items sorted to top in chat list.
